...
function sumArray(testArray){ //eslint-disable-line
  ...
  for(var i = 0; i < testArray.length; i++){
    arrSum = sum(testArray[i], arrSum)[0];
  }
  var message = [arrSum, testArray.join(',') + ' was passed in as an array of numbers, and ' + arrSum + ' is their sum.'];
  return message;
}
...
function multiplyArray(testArray){ //eslint-disable-line
  var arrProduct = 1;
  for(var i = 0; i < testArray.length; i++){
    arrProduct = multiply(testArray[i], arrProduct)[0];
  }
  var message = [arrProduct, 'The numbers ' + testArray.join(',') + ' have a product of ' + arrProduct + '.' ];
  return message;
}
